...
export default function FilteredCoursesPage() {
    ...
    const fetchCourses = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `https://backend-education-x5ta.onrender.com/api/courses/free?categoryId=${categoryId}&grade=${encodeURIComponent(
            base
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      {courses.map((course) => {
        const courseLink = `/free-courses/${categorySlug}/filter/${course.id}`;
